...
# Function to plot the robotic arm based on three angles theta1, theta2, and theta3
def plot_robotic_arm(theta1, theta2, theta3):
    # Clear the current figure
    plt.clf()

    # Set up the grid and axis settings
    ...
    # Draw a circle around the origin to represent the reach of the robotic arm
    circle = plt.Circle((0, 0), 300, color='blue', fill=False)
    plt.gca().add_patch(circle)

    # Calculate the position of points A, B, C, and D
    A = np.array([0, 0])
...
def on_click(event):
    ...
    # Check if the number of clicks is less than 21
    if len(clicks) < 21:
        # Store the clicked position
        clicks.append((x, y))

        # Draw a line from (100, 100) to the clicked position
        line, = plt.plot([100, x], [100, y], 'g-')
        plt.pause(0.25)

        # Remove the green line after 1/4th of a second
        line.remove()
        plt.draw()

        # Print table headers for the first click
        if len(clicks) == 1:
            with table:
                print("Dot# {:<7} B pos {:<10} A-B ∡ {:<4} Δ A-B ∡ {:<3} C pos {:<10.5} B-C ∡ {:<5} Δ B-C ∡ {:<3} D pos {:<10.5} C-D ∡ {:<4.5} Δ C-D ∡ {:<5} Max Δ∡".format("", "", "", "", "", "", "", "", "", "", ""))

        # Calculate and print the table values for each click
        with table:
            if len(clicks) <= 21:
                i = len(clicks)
                click = clicks[-1]
                ...
                # Calculate the maximum change in angles
                max_delta_theta = max(abs(delta_theta1), abs(delta_theta2), abs(delta_theta3))

                # Print the table values for the current click
...
# Example angles in radians for the robotic arm
theta1 = np.pi / 2
theta2 = np.pi / -2
theta3 = -np.pi / 2

# Plot the initial robotic arm position
